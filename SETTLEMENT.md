...
- [`/.well-known/agents.json`](api/_lib/agents.json) — list wrapper for that one Settlement agent (same JSON as `/api/agents.json`). Does not invent extra agents
...
- [`/api/tools.json`](api/_lib/tools.json) — callable Settlement surfaces (validate, quote, transition, simulate, verify) plus read-only discovery URLs. Not an MCP server. Not an A2A Agent Card. Not a ChatGPT plugin
...
- [`/api/schemas/quote.json`](api/_lib/quote.schema.json) — JSON Schema (2020-12) for `POST /api/v0/quote` bodies (create / fund / submit / release / dispute). Dry-run only. A quote is not an invoice and not proof of payment. Client-held demo; Liberty does not move real money
- [`/api/schemas/simulate.json`](api/_lib/simulate.schema.json) — JSON Schema (2020-12) for `POST /api/v0/simulate` bodies (one-shot create → fund → submit → release|dispute). Client-held demo protocol; Liberty does not move real money
- [`/api/schemas/verify.json`](api/_lib/verify.schema.json) — JSON Schema (2020-12) for `POST /api/v0/verify` bodies (receipt, terminal job, or submitted job plus release/dispute). Demo only. `money` is always false. A verify is not custody
- [`/api/schemas/receipt.json`](api/_lib/receipt.schema.json) — JSON Schema (2020-12) for shareable terminal receipts (`#receipt/r1.…` / verify). Client-held demo; Liberty does not move real money. Receipts are not proof of payment
- [`/api/schemas/handoff.json`](api/_lib/handoff.schema.json) — JSON Schema (2020-12) for shareable job handoffs (`#handoff/h1.…`). Client-held demo; Liberty does not move real money. Handoffs are not custody transfers
- [`/api/errors.json`](api/_lib/errors.json) — live Settlement error codes (schema vs state, including `hold_expired`). Client-held demo; Liberty does not move real money
- [`POST /api/v0/validate`](api/v0/quote.js) — dry-check of the same engine and JSON Schema; no apply, no persist, no job id (Hobby rewrite onto quote)
...
- [`/llms-full.txt`](llms-full.txt) — fuller machine-readable Settlement brief (rails, endpoints, schemas, honest zeros)
- [`/security.txt`](security.txt) / [`/.well-known/security.txt`](.well-known/security.txt) — RFC 9116 contact (public [GitHub Issues](https://github.com/masterpuzzle38/liberty/issues); demo, no SOC 2)
...
JSON body. CORS is open for `POST` and `OPTIONS`. Demo API key is optional (`Authorization: Bearer <key>` or `X-Liberty-Key`). Optional `Idempotency-Key` (or body `idempotency_key`) makes create ids stable for retries; Liberty does not replay stored responses. Validate the body against [`/api/schemas/transition.json`](api/_lib/transition.schema.json) before calling. Illegal transitions return 4xx JSON (`error`, `message`; `money` stays false).
...
Same request shape, CORS, optional demo key, and engine as transition. Validate the body against [`/api/schemas/quote.json`](api/_lib/quote.schema.json) before calling. A quote is not an invoice and not proof of payment. Dry-run only: Liberty computes the next status, `fee`, `agent_payout`, `agent_credits_delta`, `payer_credits_after`, and `returned_to_payer` when those apply, and does **not** mutate client-held state. Optional `expires_at` or `ttl_seconds` on fund stamps `expiresAt` on the quoted job (both together are rejected). After `expiresAt`, a release quote fails; dispute remains allowed. Optional `proof_note` echoes on the quoted submit job. Optional `release_note` / `dispute_reason` echo on the quoted receipt and job when sent. Response always includes `mode: "demo"`, `money: false`, and `quoted: true`. Optional `Idempotency-Key` is echoed only.
...
The human UI calls this before fund, release, and dispute so the fee / payout / credit cut is visible before confirm. Optional hold expiry chosen on the fund form is included in the fund quote.

## `POST /api/v0/simulate`

Same CORS, optional demo key, and fee engine as transition. Validate the body against [`/api/schemas/simulate.json`](api/_lib/simulate.schema.json) before calling. One-shot demo lifecycle: Liberty runs create → fund → submit → `release` or `dispute` (default `release`) through that engine. Create **assigns** a real `as_` + 10 hex id — this is a committed demo walk, not a quote. Optional `Idempotency-Key` makes that create id stable for retries. Liberty still does not persist the job or receipt.
...
The human UI on `/` can run this with safe demo defaults in one click. Optional hold expiry (TTL or absolute UTC) is sent on the fund step — same one-of-two rule as transition. A default `proof_note` is sent on submit. An optional note field is sent as `release_note` or `dispute_reason`.
...
- a `receipt` object shaped like the transition release/dispute receipt (request JSON Schema: [`/api/schemas/verify.json`](api/_lib/verify.schema.json); receipt object: [`/api/schemas/receipt.json`](api/_lib/receipt.schema.json); a top-level receipt object is also accepted), or
...
A verify is not custody. Validate the body against [`/api/schemas/verify.json`](api/_lib/verify.schema.json) before calling.
...
The snapshot is the job only. Payer credits, agent credits, the demo API key, and the receipts list stay in each browser. Copy a fresh link after each action. This is not a server-side job ledger. Validate the decoded payload against [`/api/schemas/handoff.json`](api/_lib/handoff.schema.json) before trusting a shared link. Handoffs are not custody transfers.
...
### Settlement ledger (this browser)

The human UI on [`/#ledger`](https://liberty-amber.vercel.app/#ledger) reads the Settlement localStorage already on this page — payer `{ credits, jobs }`, agent wallet, and stored receipts — and shows honest running totals for **this browser only**:

- Fees paid (sum of `release_fee` on stored receipts)
- Agent payouts credited (sum of `agent_payout` / `agent_credits_delta` on released receipts)
- Disputed returns (sum of `returned_to_payer` / disputed amounts)
- Counts of released vs disputed receipts
- Current payer credits and agent wallet balance

Download receipts as CSV (header row plus the money fields already on each receipt: amount, fee, payout, status, ids, timestamps, optional notes/refs). There is no server ledger and no receipt GET. Demo only. `money` is always false. Not real money. Not live GMV.

### Activity log (this browser)

The human UI on [`/#activity`](https://liberty-amber.vercel.app/#activity) keeps an append-only list of local demo actions in this browser (`liberty.agent-settlement.activity.v0`): create, fund, submit, release, dispute, simulate, verify, and demo pack import / export / reset. Newest first. Last 80. Job id, `client_ref`, and hold expiry appear when present.

**Clear activity** asks for confirm, then removes only that list. Wallets, jobs, receipts, and the demo API key stay. **Reset demo** does not wipe the log; it appends a reset event. Liberty never receives the log. Demo only. `money` is always false.
...
1. GET [`/.well-known/agent.json`](api/_lib/agent.json) (same JSON as [`/api/agent.json`](api/_lib/agent.json)) for the discovery card. GET [`/.well-known/agents.json`](api/_lib/agents.json) for the list wrapper that points at that card (one Settlement agent). GET [`/llms-full.txt`](llms-full.txt) for the fuller brief. GET [`/security.txt`](security.txt) for the RFC 9116 contact. GET [`/api/quickstart.json`](api/_lib/quickstart.json) for an ordered ready-to-run demo escrow walk. GET [`/api/tools.json`](api/_lib/tools.json) for callable Settlement surfaces (validate, quote, transition, simulate, verify) plus read-only discovery URLs. Not an MCP server. GET [`/api/schemas/quote.json`](api/_lib/quote.schema.json) for the quote request JSON Schema before calling `POST /api/v0/quote`. A quote is not an invoice and not proof of payment. GET [`/api/schemas/simulate.json`](api/_lib/simulate.schema.json) for the simulate request JSON Schema before calling `POST /api/v0/simulate`. GET [`/api/schemas/verify.json`](api/_lib/verify.schema.json) for the verify request JSON Schema before calling `POST /api/v0/verify`. A verify is not custody. GET [`/api/schemas/receipt.json`](api/_lib/receipt.schema.json) for the shareable receipt JSON Schema before trusting a `#receipt/r1.…` link. GET [`/api/schemas/handoff.json`](api/_lib/handoff.schema.json) for the shareable job-handoff JSON Schema before trusting a `#handoff/h1.…` link. GET [`/api/errors.json`](api/_lib/errors.json) for the live error codes (schema vs state, including `hold_expired`). GET [`/api/examples.json`](api/_lib/examples.json) for copy-ready bodies, or copy curls from `/#adapters`. GET [`/api/templates.json`](api/_lib/templates.json) for preset create-job fields (same buttons as `/#create`; fill only). GET [`/api/changelog.json`](api/_lib/changelog.json) for recent shipped slices (newest first; no user counts or revenue). GET the other JSON files for the protocol. POST `/api/v0/validate` to dry-check a body without applying; POST `/api/v0/quote` to preview the next state and fee math; POST `/api/v0/transition` to commit one demo action; POST `/api/v0/simulate` to walk create → fund → submit → release|dispute in one request; POST `/api/v0/verify` to check a receipt or proposed outcome — the same engine the human UI uses. This is not live escrow custody. Receipts are not proof of payment.
...
5. To continue a job in another browser, share a handoff link from `/` (`#handoff/h1.…`) or the compact `h1.` code. Decode is client-side. Validate the decoded payload against [`/api/schemas/handoff.json`](api/_lib/handoff.schema.json). Liberty does not persist the job. Handoffs are not custody transfers.
6. Keep a terminal receipt yourself. The transition and simulate APIs return `receipt` on release or dispute; the human UI stores that object in `localStorage` and can download JSON / NDJSON or copy a receipt link (`#receipt/r1.…`). Opening the link loads Verify. Validate the expanded object against [`/api/schemas/receipt.json`](api/_lib/receipt.schema.json) before trusting a shared link. POST `/api/v0/verify` to check fee math. Receipts are not proof of payment. `/#ledger` sums those stored receipts and can download CSV. `/#activity` lists local demo actions in this browser. Liberty does not store receipts or the activity log.
