...
test("health and settlement documents stay demo-only and match the engine", () => {
  ...
  assert.equal(SETTLEMENT.surfaces.simulate_schema, "/api/schemas/simulate.json");
  assert.equal(SETTLEMENT.simulate_schema.path, "/api/schemas/simulate.json");
  assert.equal(SETTLEMENT.simulate_schema.money, false);
  assert.equal(SETTLEMENT.simulate_api.schema, "/api/schemas/simulate.json");
  assert.equal(SETTLEMENT.surfaces.verify_schema, "/api/schemas/verify.json");
  assert.equal(SETTLEMENT.verify_schema.path, "/api/schemas/verify.json");
  assert.equal(SETTLEMENT.verify_schema.money, false);
  assert.equal(SETTLEMENT.verify_api.schema, "/api/schemas/verify.json");
  ...
  assert.equal(SETTLEMENT.activity.persistence, false);
  assert.equal(SETTLEMENT.activity.money, false);
  assert.equal(SETTLEMENT.activity.mode, "client");
  assert.equal(SETTLEMENT.activity.human_path, "/#activity");
  assert.equal(SETTLEMENT.surfaces.activity, "/#activity");
  assert.equal(SETTLEMENT.activity.storage_key, "liberty.agent-settlement.activity.v0");
  assert.equal(SETTLEMENT.activity.clear, "confirm_activity_only");
  assert.ok(SETTLEMENT.adapter_notes.some((note) => note.includes("/#activity")));
  assert.ok(SETTLEMENT.human_path.activity_storage_key === "liberty.agent-settlement.activity.v0");
