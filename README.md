...
`health.json` reports `{ "service": "liberty-agent-settlement", "mode": "demo", "money": false }`. `settlement.json` describes the state machine, fee schedule, and receipt fields. `POST /api/v0/transition` applies one action (`create`, `fund`, `submit`, `release`, `dispute`) and returns the updated job (plus credits / fee / receipt when those apply). A present demo key adds `key_id`; a missing key adds `key_optional`.
